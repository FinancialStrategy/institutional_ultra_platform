...
"Agriculture Commodities": {
    ...
    "Cocoa (Futures)": "CC=F",
    "Cotton (Futures)": "CT=F",
    "Agriculture ETF (DBA)": "DBA"
},
"Broad Commodities (ETFs)": {
    "Broad Commodities ETF (DBC)": "DBC",
    "No-K1 Broad Commodities (PDBC)": "PDBC",
    "S&P GSCI ETF (GSG)": "GSG",
    "Broad Commodities ETF (COMT)": "COMT"
}
        }
...
# ==============================================================================
# Country / Instrument Type Classification (for Universe & Data browsing)
# ==============================================================================
COUNTRY_OPTIONS = [
    "United States", "Canada", "United Kingdom", "Germany", "France", "Switzerland",
    "Turkey", "Japan", "China", "Hong Kong", "South Korea", "India", "Australia",
    "Brazil", "Mexico", "Netherlands", "Saudi Arabia", "South Africa",
    "Global", "Other"
]

INSTRUMENT_TYPE_OPTIONS = [
    "Equity", "ETF/Fund", "Index", "Rate/Yield", "FX", "Crypto", "Commodity Future", "Commodity ETF", "Other"
]

ASSET_CLASS_OPTIONS = [
    "Equity", "Fixed Income", "Rates", "Commodity", "FX", "Crypto", "Other"
]

# Common Yahoo suffix -> country mapping
_SUFFIX_COUNTRY = {
    "IS": "Turkey",
    "L": "United Kingdom",
    "DE": "Germany",
...
# Parenthetical tag -> country mapping (from your universe labels)
_NAME_COUNTRY_TAGS = {
    "US": "United States",
    "USA": "United States",
...
def _extract_parenthetical_country(name: str) -> str:
    if not name:
        return ""
    if "(" in name and ")" in name:
        inside = name.split("(")[-1].split(")")[0].strip()
        return _NAME_COUNTRY_TAGS.get(inside, inside)
    return ""
...
def infer_country_from_ticker(ticker: str) -> str:
    ...
    # Default for plain tickers (US equities/ETFs in your curated universes)
    return "United States"

def infer_country(ticker: str) -> str:
    # Try from label first (most precise in your curated lists)
    nm = TICKER_NAME_MAP.get(ticker, "")
    c = _extract_parenthetical_country(nm)
    if c:
        return c
    # Try from universe category keywords
    cat = TICKER_CATEGORY_MAP.get(ticker, "")
    cl = (cat or "").lower()
    if "bist" in cl or "turkey" in cl:
        return "Turkey"
    if "uk" in cl:
    ...
    if "australia" in cl:
        return "Australia"
    if "crypto" in cl or "fx" in cl or "commodit" in cl or "global benchmark" in cl:
        return "Global"
    return infer_country_from_ticker(ticker)

def infer_instrument_type(ticker: str) -> str:
    t = (ticker or "").upper().strip()
    cat = (TICKER_CATEGORY_MAP.get(ticker, "") or "").lower()
    nm = (TICKER_NAME_MAP.get(ticker, "") or "").lower()
    ...
    if t.startswith("^"):
        # Yield/rates (e.g., ^TNX, ^FVX) vs index
        if "yield" in nm or "rate" in nm or "rates" in cat or "fixed income" in cat:
            return "Rate/Yield"
        return "Index"
    # Category cues
    if "crypto" in cat:
    ...
    if "fx" in cat:
        return "FX"
    if "rate" in cat or "fixed income" in cat:
        # TIP/SHY/TLT etc are ETFs
        return "ETF/Fund"
    if "etf" in cat or "fund" in cat:
        return "ETF/Fund"
    if "commodity" in cat or "commodities" in cat or "precious metals" in cat or "energy" in cat or "agriculture" in cat or "industrial metals" in cat:
        # ETF tickers in these sleeves are commodity ETFs, otherwise futures already handled
        # Heuristic: common ETF patterns are 2-5 letters, no dot, no special chars
        if t.isalpha() and 2 <= len(t) <= 5:
            return "Commodity ETF"
        return "Commodity Future"
    # Default
    # Simple heuristic: short all-caps tickers could be ETFs; but we keep as Equity unless we know it's an ETF sleeve
    return "Equity"

def infer_asset_class(ticker: str) -> str:
    it = infer_instrument_type(ticker)
    if it in {"Equity"}:
        return "Equity"
    if it in {"ETF/Fund"}:
        # If the label/category indicates bonds/rates -> Fixed Income
        cat = (TICKER_CATEGORY_MAP.get(ticker, "") or "").lower()
        nm = (TICKER_NAME_MAP.get(ticker, "") or "").lower()
        if "bond" in nm or "treasury" in nm or "fixed income" in cat or "rates" in cat or ticker in {"TLT","IEF","AGG","BND","LQD","HYG","JNK","TIP","SHY"}:
            return "Fixed Income"
        if "commodity" in cat or "precious metals" in cat:
            return "Commodity"
        return "Equity"
    if it in {"Index"}:
        return "Equity"
    ...
    if it in {"Crypto"}:
        return "Crypto"
    if it in {"Commodity Future", "Commodity ETF"}:
        return "Commodity"
    return "Other"
...
def ticker_meta(ticker: str) -> Dict[str, str]:
    return {
        ...
        "UniverseCategory": TICKER_CATEGORY_MAP.get(ticker, "Custom/Unknown"),
        "Region": infer_region(ticker),
        "Country": infer_country(ticker),
        "InstrumentType": infer_instrument_type(ticker),
        "AssetClass": infer_asset_class(ticker),
    }

def meta_df_for_tickers(tickers: List[str]) -> pd.DataFrame:
    rows = [ticker_meta(t) for t in (tickers or [])]
    df = pd.DataFrame(rows)
    if not df.empty:
        # stable ordering
        cols = ["Ticker", "Name", "UniverseCategory", "Region", "Country", "InstrumentType", "AssetClass"]
        df = df[[c for c in cols if c in df.columns]]
    return df

def build_universe_indexes(all_tickers: List[str]) -> Dict[str, Dict[str, List[str]]]:
    idx: Dict[str, Dict[str, List[str]]] = {"Region": {}, "Country": {}, "Instrument Type": {}}
    for t in all_tickers:
        r = infer_region(t)
        c = infer_country(t)
        it = infer_instrument_type(t)

        idx["Region"].setdefault(r, []).append(t)
        idx["Country"].setdefault(c, []).append(t)
        idx["Instrument Type"].setdefault(it, []).append(t)

    # sort and de-duplicate
    for k in idx:
        for g in list(idx[k].keys()):
            idx[k][g] = sorted(list(dict.fromkeys(idx[k][g])))
    return idx

# Precompute a browse index over your curated universe tickers
ALL_KNOWN_TICKERS = sorted(set(TICKER_NAME_MAP.keys()) | set(TICKER_CATEGORY_MAP.keys()))
UNIVERSE_INDEX = build_universe_indexes(ALL_KNOWN_TICKERS)
...
def main():
    ...
    browse_mode = st.sidebar.selectbox(
        "Browse Universe by",
        ["Curated Universes", "Region", "Country", "Instrument Type"],
        index=0
    )

    # Curated universes (your original menu) + alternative views (Region/Country/Instrument Type)
    if browse_mode == "Curated Universes":
        ...
        options = list(universe_dict.values())
    else:
        if browse_mode == "Region":
            groups = sorted(list(UNIVERSE_INDEX.get("Region", {}).keys()))
            group = st.sidebar.selectbox("Region", groups) if groups else "Global"
            category = f"Region: {group}"
            options = UNIVERSE_INDEX.get("Region", {}).get(group, [])
        elif browse_mode == "Country":
            groups = sorted(list(UNIVERSE_INDEX.get("Country", {}).keys()))
            group = st.sidebar.selectbox("Country", groups) if groups else "Global"
            category = f"Country: {group}"
            options = UNIVERSE_INDEX.get("Country", {}).get(group, [])
        else:
            groups = sorted(list(UNIVERSE_INDEX.get("Instrument Type", {}).keys()))
            group = st.sidebar.selectbox("Instrument Type", groups) if groups else "Equity"
            category = f"Instrument Type: {group}"
            options = UNIVERSE_INDEX.get("Instrument Type", {}).get(group, [])

        universe_dict = {TICKER_NAME_MAP.get(t, t): t for t in options}

    if not options:
        st.sidebar.warning("No tickers found for this classification. Switch browse mode or add custom tickers below.")
        options = []

    default_n = 8 if browse_mode == "Curated Universes" else 12
    default_tickers = list(options)[:min(default_n, len(options))]

    tickers = st.sidebar.multiselect(
        "Select assets (50+ supported)",
        options=options,
    ...
    st.sidebar.markdown("#### Add custom tickers (optional)")
    custom_raw = st.sidebar.text_area("Extra tickers (comma/newline-separated)", value="", height=70)
    if custom_raw.strip():
        _clean = ",".join(custom_raw.splitlines())
        extras = [s.strip() for s in _clean.split(",")]
        extras = [e for e in extras if e]
        tickers = list(dict.fromkeys(list(tickers) + extras))
