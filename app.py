...
class AdvancedPerformanceMetrics:
    ...
    def calculate_all_metrics(self) -> Dict:
        ...
        jb_stat, jb_pval = self._calculate_jarque_bera()
        metrics["Jarque-Bera Stat"] = float(jb_stat)
        metrics["Jarque-Bera p-value"] = float(jb_pval)
        metrics["Normality (JB p>0.05)"] = "Normal" if jb_pval > 0.05 else "Non-Normal"
        ...
        if self.benchmark_aligned is not None:
            ...
            metrics["Jensen Alpha"] = float(alpha)  # Changed from "Jensen's Alpha" to avoid apostrophe issues
            ...
            if "Down Capture Ratio" in metrics and metrics["Down Capture Ratio"] != 0:
                metrics["Capture Ratio"] = metrics["Up Capture Ratio"] / metrics["Down Capture Ratio"]
            else:
                metrics["Capture Ratio"] = float("nan")
        ...
        if metrics["Average Loss"] != 0:
            metrics["Win/Loss Ratio"] = abs(metrics["Average Win"] / metrics["Average Loss"])
        else:
            metrics["Win/Loss Ratio"] = float("inf")
        
        if negative_returns.sum() != 0:
            metrics["Profit Factor"] = float(positive_returns.sum() / abs(negative_returns.sum()))
        else:
        ...
        if pain > 0:
            metrics["Gain to Pain Ratio"] = float(gain / pain)
        else:
            metrics["Gain to Pain Ratio"] = float("inf")
        ...
        if len(recovery_days) > 0:
            metrics["Avg Recovery Days"] = float(np.mean(recovery_days))
            metrics["Max Recovery Days"] = int(np.max(recovery_days))
        else:
            metrics["Avg Recovery Days"] = 0.0
            metrics["Max Recovery Days"] = 0
        
        # 8. ADDITIONAL METRICS
        metrics["Ulcer Index"] = float(np.sqrt(np.mean(drawdown_series ** 2)))
        if metrics["Maximum Drawdown"] < 0:
            metrics["Sterling Ratio"] = float(metrics["Annual Return"] / metrics["Maximum Drawdown"])
        else:
            metrics["Sterling Ratio"] = 0.0
        
        if np.sum(np.square(drawdown_series)) > 0:
            metrics["Burke Ratio"] = float(metrics["Annual Return"] / np.sqrt(np.sum(np.square(drawdown_series))))
        else:
            metrics["Burke Ratio"] = 0.0
    ...
    def format_views_for_display(self, views: List, confidences: List) -> pd.DataFrame:
        ...
        for i, (view, conf) in enumerate(zip(views, confidences)):
            if view["type"] == "absolute":
                ...
                view_desc = f"{asset_name} will return {view['return']:.1%}"
                view_data.append({
                    "View #": i+1,
                    "Type": "Absolute",
                ...
                view_desc = f"{asset1} will outperform {asset2} by {view['outperformance']:.1%}"
                view_data.append({
                    "View #": i+1,
                    "Type": "Relative",
                    "Description": view_desc,
...
class EnhancedRiskMetricsEngine:
    ...
    def calculate_comprehensive_metrics(self) -> pd.DataFrame:
        ...
        if abs(metrics["Max Drawdown"]) > 0:
            metrics["Calmar Ratio"] = float(metrics["Annual Return"] / abs(metrics["Max Drawdown"]))
        else:
        ...
        if losses > 0:
            metrics["Omega Ratio"] = float(gains / losses)
        else:
            metrics["Omega Ratio"] = float("inf")
        ...
        if len(self.returns) >= 30:
            ...
            if len(tail) > 0:
                metrics["CVaR (95%)"] = float(-tail.mean())
            else:
...
class AdvancedVaREngine:
    ...
    def historical_var(self, confidence: float = 0.95, holding_period: int = 1) -> Dict:
        ...
        if len(losses_beyond_threshold) > 0:
            cvar = -losses_beyond_threshold.mean()
        else:
            cvar = np.nan
...
def create_advanced_performance_tab():
    ...
    with col_ret1:
        annual_return = all_metrics.get("Annual Return", 0)
        st.metric("Annual Return", f"{annual_return:.2%}")
    with col_ret2:
        cumulative_return = all_metrics.get("Cumulative Return", 0)
        st.metric("Cumulative Return", f"{cumulative_return:.2%}")
    with col_ret3:
        geometric_return = all_metrics.get("Geometric Mean Return", 0)
        st.metric("Geometric Mean", f"{geometric_return:.2%}")
    with col_ret4:
        rolling_3m = all_metrics.get("3M Rolling Return", 0)
        st.metric("3M Rolling Return", f"{rolling_3m:.2%}")
    ...
    with col_risk1:
        annual_vol = all_metrics.get("Annual Volatility", 0)
        st.metric("Annual Volatility", f"{annual_vol:.2%}")
    with col_risk2:
        max_dd = all_metrics.get("Maximum Drawdown", 0)
        st.metric("Maximum Drawdown", f"{max_dd:.2%}")
    with col_risk3:
        var_95 = all_metrics.get("VaR 95% (Historical)", 0)
        st.metric("VaR 95%", f"{var_95:.2%}")
    with col_risk4:
        cvar_95 = all_metrics.get("CVaR/ES 95%", 0)
    ...
    with col_ratio1:
        sharpe = all_metrics.get("Sharpe Ratio", 0)
        st.metric("Sharpe Ratio", f"{sharpe:.2f}")
    with col_ratio2:
        sortino = all_metrics.get("Sortino Ratio", 0)
        st.metric("Sortino Ratio", f"{sortino:.2f}")
    with col_ratio3:
        calmar = all_metrics.get("Calmar Ratio", 0)
    ...
    if benchmark_returns is not None and len(benchmark_returns) > 0:
        ...
        with col_bench2:
            alpha = all_metrics.get("Jensen Alpha", 0)
            st.metric("Alpha", f"{alpha:.2%}")
        with col_bench3:
            info_ratio = all_metrics.get("Information Ratio", 0)
            st.metric("Information Ratio", f"{info_ratio:.2f}")
        with col_bench4:
            r_squared = all_metrics.get("R-squared", 0)
    ...
    metrics_by_category = {
        ...
        "Benchmark Metrics": ["Benchmark Return", "Excess Return", "Beta", "Jensen Alpha",
...
def create_advanced_frontier_tab():
                ...
                if i == 0:
                    with col_key1:
                        ...
                        st.metric(
                            ...
                            f"Vol: {portfolio_vol:.2%}, Sharpe: {portfolio_sharpe:.2f}"
                        )
                elif i == 1:
                    with col_key2:
                        portfolio_return = portfolio['return']
    ...
    if st.button("🔍 Find Optimal Portfolio for Target", use_container_width=True):
        ...
        with col_sim_res1:
            closest_return = closest_portfolio['return']
            st.metric("Expected Return", f"{closest_return:.2%}")
        with col_sim_res2:
            closest_vol = closest_portfolio['volatility']
            st.metric("Expected Volatility", f"{closest_vol:.2%}")
        with col_sim_res3:
            closest_sharpe = closest_portfolio['sharpe']
            st.metric("Sharpe Ratio", f"{closest_sharpe:.2f}")
...
def create_var_analysis_tab():
    ...
    with st.spinner("🔬 Calculating advanced risk metrics..."):
        ...
        if var_method == "Historical":
            ...
            # For now, use historical as placeholder
            primary_result = var_engine.historical_var(confidence_level, holding_period)
    ...
    with col_metric2:
        ...
        if not np.isnan(cvar_pct):
            cvar_amount = cvar_pct / 100 * investment_amount
            st.metric(
    ...
    with col_metric4:
        if not np.isnan(cvar_pct):
            tail_risk = cvar_pct - var_pct
            st.metric("Tail Risk (CVaR - VaR)", f"{tail_risk:.2f}%")
        else:
            st.metric("Tail Risk (CVaR - VaR)", "N/A")
...
def create_enhanced_stress_test_tab():
    ...
    st.markdown('<div class="subsection-header">📜 Historical Financial Crises</div>', unsafe_allow_html=True)
    ...
    selected_crises = st.multiselect(
        ...
        default=["COVID-19 Pandemic Crash (2020)"],
...
def main():
    ...
    with tab8:
        ...
        if "portfolio_returns" not in st.session_state:
            ...
            amt_for_var = float(st.session_state.get("last_amount", 1000000))
            
            bench_ret = None
            if "benchmark_returns" in st.session_state:
                bench_ret = st.session_state.benchmark_returns
            ...
            with col_met2:
                ...
                for _, row in other_metrics.iterrows():
                    metric_name = row["Metric"]
                    metric_value = row["Value"]
                    if ("Ratio" in metric_name) or ("Beta" in metric_name) or ("Alpha" in metric_name):
                        if np.isfinite(metric_value):
                            st.metric(metric_name, f"{metric_value:.3f}")
                        else:
                            st.metric(metric_name, "∞")
